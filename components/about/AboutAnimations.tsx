...
import { Award, Target, Users, Factory, Truck, Box, Sofa, FlaskConical, Monitor, Hammer } from 'lucide-react';
...
const industriesConfig = [
    { name: "FMCG & F&B", icon: Factory, color: "text-blue-500", bg: "bg-blue-50", border: "border-blue-200" },
    { name: "Otomotif & Kendaraan", icon: Truck, color: "text-red-500", bg: "bg-red-50", border: "border-red-200" },
    { name: "Kimia & Farmasi", icon: FlaskConical, color: "text-emerald-500", bg: "bg-emerald-50", border: "border-emerald-200" },
    { name: "Kaca & Material Solid", icon: Box, color: "text-slate-500", bg: "bg-slate-50", border: "border-slate-200" },
    { name: "Kemasan & Plastik", icon: Box, color: "text-indigo-500", bg: "bg-indigo-50", border: "border-indigo-200" },
    { name: "Furnitur & Interior", icon: Sofa, color: "text-amber-500", bg: "bg-amber-50", border: "border-amber-200" },
    { name: "Bahan Bangunan & Infrastruktur", icon: Hammer, color: "text-stone-500", bg: "bg-stone-50", border: "border-stone-200" },
    { name: "Industri Berat & Otomasi", icon: Factory, color: "text-slate-700", bg: "bg-slate-100", border: "border-slate-300" },
    { name: "Elektronik", icon: Monitor, color: "text-cyan-500", bg: "bg-cyan-50", border: "border-cyan-200" },
    { name: "Logistik Udara", icon: Truck, color: "text-sky-500", bg: "bg-sky-50", border: "border-sky-200" }
];
        ...
        <div className="flex flex-col gap-12 max-w-7xl mx-auto w-full px-4">
            {industriesConfig.map((industry, catIndex) => {
                ...
                return (
                    ...
                    >
                        {/* Industry Header (Glassmorphism & Elegant) */}
                        <div className="flex items-center gap-4 mb-6 relative">
                            <div className="flex-1 h-px bg-gradient-to-r from-transparent via-slate-200 to-transparent absolute inset-0 top-1/2 -z-10"></div>
                            <div className={`inline-flex items-center gap-2 px-5 py-2 rounded-full border ${industry.border} ${industry.bg} shadow-sm backdrop-blur-md bg-opacity-90 mx-auto md:mx-0`}>
                                <Icon size={16} className={industry.color} />
                                <h3 className="text-sm font-bold text-slate-800 uppercase tracking-widest">{industry.name}</h3>
                                <span className="ml-2 px-2 py-0.5 rounded-full bg-white text-[10px] font-black text-slate-500 shadow-sm border border-slate-100">
                                    {logos.length}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 md:gap-6">
                            {logos.map((logo, index) => (
                                <motion.div
                                    ...
                                    className="relative h-24 w-full flex items-center justify-center p-4 group bg-white rounded-2xl border border-slate-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:border-slate-200 hover:z-10 transition-all duration-300"
                                >
                                    {/* Tooltip on Hover */}
                                    ...
                                    <img
                                        src={logo.logo}
                                        alt={logo.client}
                                        className="object-contain grayscale opacity-50 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-500 max-h-12 w-auto max-w-full drop-shadow-sm group-hover:drop-shadow-md"
                                    />
                                </motion.div>
                            ))}
                        </div>
