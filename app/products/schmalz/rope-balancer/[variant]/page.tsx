...
export async function generateStaticParams() {
    return ropeBalancerVariants.map(v => ({ variant: v.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
    const { variant: slug } = await params;
    const v = ropeBalancerVariants.find(x => x.slug === slug);
    if (!v) return { title: 'Not Found' };
    return { title: `${v.name} | Rope Balancers Schmalz — PT Dimensi Quantum Wahyudi`, description: v.description.slice(0, 160) };
}
...
export default async function RopeBalancerVariantPage({ params }: Props) {
    const { variant: slug } = await params;
    const variant = ropeBalancerVariants.find(v => v.slug === slug);
    if (!variant) notFound();
    ...
    const productSchema = {
        ...
        category: 'Rope Balancer',
        url: `https://dimensiwahyudi.com/products/schmalz/rope-balancer/${variant.slug}`,
    ...
    const breadcrumbSchema = {
        ...
        itemListElement: [
            ...
            { '@type': 'ListItem', position: 4, name: 'Rope Balancer', item: 'https://dimensiwahyudi.com/products/schmalz/rope-balancer' },
            { '@type': 'ListItem', position: 5, name: variant.name, item: `https://dimensiwahyudi.com/products/schmalz/rope-balancer/${variant.slug}` },
        ],
    };
            ...
            <section className="relative pt-28 pb-14 bg-slate-950 overflow-hidden">
                ...
                <div className="relative max-w-7xl mx-auto px-6">
                    <nav className="flex items-center gap-2 text-slate-500 text-xs mb-8 flex-wrap">
                        ...
                        <ChevronRight size={12} />
                        <Link href="/products/schmalz/rope-balancer" className="hover:text-amber-400 transition-colors">Rope Balancers</Link>
                        <ChevronRight size={12} />
                        <span className="text-slate-300">{variant.seriesCode}</span>
                    </nav>
                    <div className="flex items-center gap-2 flex-wrap mb-3">
                        <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded-full text-amber-400 text-xs font-bold uppercase tracking-widest">
                            Binar Handling
                        </div>
                        {variant.badge && (
                    ...
                    <p className="text-amber-400 font-bold text-lg max-w-2xl">{variant.tagline}</p>
                </div>
            </section>
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            ...
                            <div className="bg-white rounded-2xl border border-slate-200 p-8">
                                ...
                                <div className="mt-6 flex items-start gap-3 p-4 bg-amber-50 rounded-xl border border-amber-100">
                                    ...
                                    <div>
                                        <div className="text-xs font-black text-amber-700 uppercase tracking-wider mb-1">Primary Advantage</div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-white rounded-2xl border border-slate-200 p-6 sticky top-24">
                                ...
                                <div className="mt-6 pt-6 border-t border-slate-100">
                                    <div className="text-xs text-slate-400 font-medium mb-3 uppercase tracking-wider">Other Rope Balancer Variants</div>
                                    {ropeBalancerVariants.filter(v => v.slug !== slug).map(v => (
                                        <Link key={v.slug} href={`/products/schmalz/rope-balancer/${v.slug}`}
                                            className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-slate-50 transition-colors group/s">
                                            <div>
                                                <div className="text-slate-700 text-xs font-bold group-hover/s:text-amber-700">{v.seriesCode}</div>
                                                <div className="text-slate-400 text-[10px]">{v.capacity}</div>
                                            </div>
                                            <ChevronRight size={12} className="text-slate-400" />
                                        </Link>
                                    ))}
                                    <Link href="/products/schmalz/rope-balancer"
                                        className="flex items-center gap-1.5 py-2 px-3 text-xs font-bold text-amber-600 hover:bg-amber-50 rounded-lg transition-colors mt-1">
                                        View all variants ({ropeBalancerVariants.length})
                                    </Link>
                                </div>
                            </div>
                        </div>
