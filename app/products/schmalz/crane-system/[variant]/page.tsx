...
export async function generateStaticParams() {
    return craneSystemVariants.map(v => ({ variant: v.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
    const { variant: slug } = await params;
    const v = craneSystemVariants.find(x => x.slug === slug);
    if (!v) return { title: 'Not Found' };
    return { title: `${v.name} | Crane Systems Schmalz — PT Dimensi Quantum Wahyudi`, description: v.description.slice(0, 160) };
}
...
export default async function CraneSystemVariantPage({ params }: Props) {
    const { variant: slug } = await params;
    const variant = craneSystemVariants.find(v => v.slug === slug);
    if (!variant) notFound();
    ...
    const productSchema = {
        ...
        url: `https://dimensiwahyudi.com/products/schmalz/crane-system/${variant.slug}`,
    ...
    const breadcrumbSchema = {
        ...
        itemListElement: [
            ...
            { '@type': 'ListItem', position: 4, name: 'Crane Systems', item: 'https://dimensiwahyudi.com/products/schmalz/crane-system' },
            { '@type': 'ListItem', position: 5, name: variant.name, item: `https://dimensiwahyudi.com/products/schmalz/crane-system/${variant.slug}` },
        ],
    };
            ...
            <section className="relative pt-28 pb-14 bg-slate-950 overflow-hidden">
                ...
                <div className="relative max-w-7xl mx-auto px-6">
                    <nav className="flex items-center gap-2 text-slate-500 text-xs mb-8 flex-wrap">
                        ...
                        <ChevronRight size={12} />
                        <Link href="/products/schmalz/crane-system" className="hover:text-blue-400 transition-colors">Crane Systems</Link>
                        <ChevronRight size={12} />
                    ...
                    <div className="flex items-center gap-2 flex-wrap mb-3">
                        ...
                        {variant.badge && (
                            <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest ${badgeColors[variant.badge] || 'bg-slate-600 text-white'}`}>
                                <Shield size={10} /> {variant.badge}
                            </div>
                        )}
                    </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-white rounded-2xl border border-slate-200 p-6 sticky top-24">
                                ...
                                <div className="mt-6 pt-6 border-t border-slate-100">
                                    <div className="text-xs text-slate-400 font-medium mb-3 uppercase tracking-wider">Other Crane System Variants</div>
                                    {craneSystemVariants.filter(v => v.slug !== slug).map(v => (
                                        <Link key={v.slug} href={`/products/schmalz/crane-system/${v.slug}`}
                                    ...
                                    <Link href="/products/schmalz/crane-system"
                                        ...
                                        View all variants ({craneSystemVariants.length})
                                    </Link>
                                </div>
                            </div>
                        </div>
